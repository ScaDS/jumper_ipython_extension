...
# The replayed cells live in {body_module}, imported only under this guard. A
# cell that starts worker processes - dask, multiprocessing, joblib - makes
# Python's spawn start method re-import this file in every child. Unguarded,
# each child replays the whole prefix and tries to start workers of its own,
# until multiprocessing's bootstrapping check kills it and the parent reports
# only "failed to start worker process". Keeping the body in a module of its
...
# Guarded because a prefix cell may start worker processes: Python's spawn start
# method re-imports this file in every child, and unguarded the child would
# replay the whole prefix there. Every cell reaches this script as a repr on one
# line, so indenting under the guard cannot disturb what it contains.
if __name__ == "__main__":
...
# Guarded for the same reason the checkpoint is: the timed cell may start worker
# processes, and spawn re-imports this file in every one of them.
if __name__ == "__main__":
...
def build_script(
    ...
    The cells go in a body module beside the entry script rather than in it, so
    that the spawn re-import of the entry script in a worker child finds nothing
    to replay. See ``_ENTRY``.
    """
    ...
    body_path = body_path_for(output_path)
    _write(body_path, "".join(parts))
    return _write(output_path, _ENTRY.format(body_module=Path(body_path).stem))
...
def failing_prefix_cell(script_path: str, error: str) -> int | None:
    ...
    to blame.
    """
    # The replayed cells live in the body module beside the entry script, so a
    # prefix traceback names that file rather than the one that was invoked.
    body_path = body_path_for(script_path)
    if Path(body_path).exists():
        script_path = body_path
